...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const displayLimit = Number(body?.display);

    if (!query) {
      throw new Error("검색어(query)가 필요합니다.");
    }

    const display = Number.isFinite(displayLimit)
      ? Math.min(Math.max(Math.floor(displayLimit), 1), 20)
      : 5;

    const articles = await fetchNaverNews(query, clientId, clientSecret, display);
