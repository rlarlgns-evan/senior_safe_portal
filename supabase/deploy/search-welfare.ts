...
function regionNameToCode(regionName: string): string {
  const raw = regionName.replace(/\s+/g, "");
  const normalized = normalizeRegionInput(regionName);

  for (const key of CTPRVN_KEYS_BY_LENGTH) {
    const entry = CTPRVN_CODES[key];
    if (raw.includes(key) || normalized.includes(key)) return entry.code;

    const keyNorm = normalizeAreaName(key);
    const nameNorm = normalizeAreaName(entry.name);
    const textNorm = normalizeAreaName(normalized);
    if (textNorm === nameNorm || textNorm === keyNorm) return entry.code;
    if (nameNorm.length >= 2 && textNorm.startsWith(nameNorm.slice(0, 2))) return entry.code;
  }
...
function serviceMatchesRegion(
  ...
  const serviceCode = regionNameToCode(service.region);
  if (serviceCode && codes.ctprvnCd) {
    return serviceCode === codes.ctprvnCd;
  }
  ...
  return serviceRegionNorm.slice(0, 2) === targetRegionNorm.slice(0, 2);
}
...
async function fetchNationalWelfareServices(
  ...
  const perPage = Math.min(Math.max(limit * 5, 30), 100);

  for (const path of paths) {
    if (collected.size >= limit) break;

    try {
      const { items } = await fetchOdcloudPage(serviceKey, path, 1, perPage);
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const listSize = preview
      ? Math.max(serviceLimit * 2, 8)
      : category === "all"
        ? Math.max(serviceLimit * 3, 8)
        : Math.max(serviceLimit * 4, 24);
    const summaries = await fetchWelfareList(serviceKey, codes, listSize);

    const detailCap = preview ? serviceLimit + 2 : serviceLimit * 2;
    const detailTargets = summaries.slice(0, detailCap);
    ...
    let nationalServices: NationalWelfareService[] = [];
    try {
      const nationalLimit = preview ? serviceLimit : category === "all" ? serviceLimit : serviceLimit * 2;
      const nationalRaw = await fetchNationalWelfareServices(serviceKey, nationalLimit);
      nationalServices = nationalRaw
        .filter((service) => matchesWelfareCategory(service, category))
        .slice(0, serviceLimit);
    } catch (nationalError) {
      console.error("national welfare fetch failed:", nationalError);
    }
